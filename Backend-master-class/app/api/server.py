...
class Server:
    def __init__(self, host, port, db_host, db_port, user, password, db_name, rebuild_db=False):
        ...
        self.app.add_url_rule('/add_user_info', view_func=self.add_user_info, method=['POST'])
        self.app.add_url_rule('/get_user_info/<username>',  view_func=self.get_user_info)   # method=['GET']
    ...
    def run_server(self):
        self.server = threading.Thread(target=self.app.run, kwargs={
                                       'host': self.host, 
                                       'port': self.port})
        self.server.start()
        return self.server
